...
const InsidenTable = ({ setChartData }) => {
  ...
  const fetchIncidents = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/insidens`
      );
  ...
  const calculateElapsedTime = (insiden) => {
    ...
    const startTime = insiden.tanggalReopen
      ? new Date(insiden.tanggalReopen)
      : new Date(insiden.tanggalSubmit);
  ...
  const fetchChats = async (incidentId) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/insidens/${incidentId}/chat`
      );
  ...
  const handleCloseIncident = async (incident) => {
    try {
      ...
      const startTime = incident.tanggalReopen
        ? new Date(incident.tanggalReopen)
        : new Date(incident.tanggalSubmit);
      ...
      const totalElapsedTime =
        (incident.elapsedTime || 0) + elapsedMilliseconds;
      ...
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/insidens/close/${incident._id}`,
        {
  ...
  const handleReopenIncident = async (incident) => {
    try {
      ...
      const formattedElapsedTime = `${formatElapsedTime(
        incident.elapsedTime
      )} + ${formatElapsedTime(elapsedSinceClose)} (Reopened)`;

      // Kirim request untuk membuka kembali insiden dan menyimpan waktu reopen
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/insidens/reopen/${incident._id}`,
        {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      const updatedElapsedTimes = filteredInsidens.map((insiden) =>
        calculateElapsedTime(insiden)
      );
      ...
      if (gridApi) {
        ...
        filteredInsidens.forEach((insiden, index) => {
          ...
          if (insiden.status !== "Closed") {
            // Only update if the status is not closed
  ...
  const handleDeleteIncident = async (incidentId) => {
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/insidens/${incidentId}`
      );
  ...
  const downloadCSV = () => {
    ...
    const csvRows = filteredInsidens.map((insiden, index) => {
      const elapsedTime =
        insiden.status === "Closed" ? insiden.elapsedTime : elapsedTimes[index]; // Get elapsed time
  ...
  const downloadExcel = () => {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
      // Using aoa_to_sheet to insert headers
    ...
    reader.onload = async (event) => {
      try {
        ...
        for (const row of jsonData) {
          ...
          // Helper function to handle empty or '-' values
          const checkValue = (value) => (value && value !== "-" ? value : "-");

          // Convert date strings from the Excel file to Date objects, with validation
          const startDate = row["Tanggal Start"]
            ? addGMT7(new Date(formatDate(row["Tanggal Start"])))
            : gmt7Date;
          const submitDate = row["Tanggal Submit"]
            ? addGMT7(new Date(formatDate(row["Tanggal Submit"])))
            : gmt7Date;

          // Calculate elapsed time (in milliseconds)
          // Parse Waktu Yang Dibutuhkan (e.g., '3d 18h 1m 50s') and convert it to milliseconds
          let elapsedTime = parseElapsedTime1(row["Durasi"]);

          // Prepare new incident object
          const newIncident = {
            idInsiden: checkValue(row["ID Ticket"]),
            deskripsi: checkValue(row["Deskripsi"]),
            status: checkValue(row["Status"]),
            sbu: checkValue(row["SBU"]),
            pilihan: checkValue(row["Pilihan"]),
            tanggalStart: submitDate,
            tanggalSubmit: startDate,
            priority: checkValue(row["Priority"]),
          ...
          if (!newIncident.idInsiden || !newIncident.deskripsi) {
            throw new Error(
              "Invalid data format in the Excel sheet. Please check the required fields (ID Ticket, Deskripsi)."
            );
          }
  ...
  const formatDate = (dateString) => {
    ...
    const formattedDate = `${datePart[2]}-${datePart[1].padStart(
      2,
      "0"
    )}-${datePart[0].padStart(2, "0")}`;
  ...
  const handleEditIncident = async (updatedIncident) => {
    try {
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/insidens/${updatedIncident._id}`,
        updatedIncident
      );
      fetchIncidents();
  ...
  const updateIncidentStatus = async (incidentId) => {
    return axios.put(
      `${process.env.REACT_APP_API_URL}/api/insidens/work/${incidentId}`,
      { status: "In Queue" }
    );
  };
  ...
  const handleDeleteSelectedIncidents = async () => {
    ...
    try {
      ...
      const deletePromises = selectedRows.map(async (row) => {
        try {
          await axios.delete(
            `${process.env.REACT_APP_API_URL}/api/insidens/${row._id}`
          );
        } catch (error) {
          // Tangkap dan log error dari setiap permintaan individu
          console.error(
            `Error deleting incident with ID ${row.idInsiden}:`,
            error
          );
          const errorMessage =
          ...
          alert(
            `Failed to delete incident with ID ${row.idInsiden}: ${errorMessage}`
          );
        }
      });
      ...
      const errorMessage =
        error.response?.data?.message ||
        "An unknown error occurred while deleting the selected incidents.";
    ...
    <div style={containerStyle}>
      ...
      <div className="ag-theme-alpine" style={gridStyle}>
        <div className="filter-bar">
          ...
          <select
            value={elapsedFilter}
            onChange={handleElapsedFilterChange}
            className="elapsed-filter"
          >
        ...
        {showDetailsModal && modalData && (
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <p>
                <strong>ID Insiden:</strong> {modalData.idInsiden}
              </p>
              <p>
                <strong>Deskripsi:</strong> {modalData.deskripsi}
              </p>
              <p>
                <strong>Status:</strong> {modalData.status}
              </p>
              <p>
                <strong>Start Date:</strong> {modalData.tanggalSubmit}
              </p>
              <p>
                <strong>SBU:</strong> {modalData.sbu}
              </p>
              <p>
                <strong>Kategori:</strong> {modalData.pilihan}
              </p>
