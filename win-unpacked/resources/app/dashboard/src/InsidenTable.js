...
const InsidenTable = ({ setChartData }) => {
    ...
    const handleReopenIncident = async (incident) => {
        try {
            ...
            await axios.put(`${process.env.REACT_APP_API_URL}/api/insidens/reopen/${incident._id}`, {
    ...
    const handleDeleteIncident = async (incidentId) => {
        try {
            await axios.delete(`${process.env.REACT_APP_API_URL}/api/insidens/${incidentId}`);
        ...
        reader.onload = async (event) => {
            try {
                ...
                const checkValue = (value) => (value && value !== '-' ? value : '-');
                    ...
                    await axios.post(`${process.env.REACT_APP_API_URL}/api/insidens`, newIncident);
                }
    ...
    const handleEditIncident = async (updatedIncident) => {
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/api/insidens/${updatedIncident._id}`, updatedIncident);
            fetchIncidents();
    ...
    try {
        ...
        const deletePromises = selectedRows.map(async (row) => {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/insidens/${row._id}`);
            } catch (error) {
